...
func nativeMethodHandlers() map[string]map[string]nativeMethodHandler {
	return map[string]map[string]nativeMethodHandler{
		...
		"Either": {
			...
			"expectRight": nativeEitherExpectRight,
			"expectLeft":  nativeEitherExpectLeft,
			"getOr":       nativeEitherGetOr,
			"map":         nativeEitherMap,
			"mapLeft":     nativeEitherMapLeft,
			"flatMap":     nativeEitherFlatMap,
			"toOption":    nativeEitherToOption,
			"toResult":    nativeEitherToResult,
		},
		"Set": {
			"add":      nativeSetAdd,
